# Design Document: Canon as Primary Development Context

**Date:** 2026-02-19
**Status:** Draft / Exploration
**Related:** [Technical Reference](./canon-ide-technical-reference.md) | [Subscription Integration](./claude-subscription-integration.md)

---

## Vision

Invert Canon's relationship with Claude Code. Instead of Canon being an extension launched from Claude Code (`/canon:new`), Canon becomes the primary development context — a browser-based workspace where Claude Code sessions run *inside* Canon.

The core interaction model stays the same: **annotation-first development**. Users review code and conversation output, attach spatially-anchored feedback, and submit structured annotations that direct Claude's next actions. What changes is scope — annotations extend from code files to conversation output, and Canon manages the full lifecycle of Claude Code sessions rather than being a one-shot review tool.
...
Canon remains a **review and orchestration layer**. Code editing stays in Claude's hands (or the user's existing editor). Canon shows code read-only, as it does today, and adds the ability to see, annotate, and direct Claude Code conversations.
...
The proposed model extends annotations to target three surfaces: code files, conversation messages, and tool calls. Each target carries the spatial context needed to anchor the annotation precisely. See [Technical Reference: Extended Annotation Model](./canon-ide-technical-reference.md#typescript-interfaces) for the interface definitions and XML format.
...
If Anthropic ever opens the SDK to subscription auth, migration from CLI subprocess to SDK would be straightforward — the message shapes are similar, and Canon's rendering layer is format-agnostic.

See [Subscription Integration](./claude-subscription-integration.md) for the full auth analysis and [Technical Reference: Spawning Sessions](./canon-ide-technical-reference.md#spawning-sessions) for implementation details.

#### 2. Persistent Server, Not One-Shot

Canon becomes a long-running process. The current `waitForDecision()` → exit pattern is replaced with a persistent Bun server that:
...
The existing one-shot `/canon:new` slash command and `waitForDecision()` pattern will be removed as part of the migration. Canon becomes a persistent-only tool.
...
4. Submit — Canon composes the structured prompt from your annotations
...
### Session Initiation
- How does a user start a new Claude Code session from Canon? A button? A prompt input? Both?
- What's the default state when Canon first opens — empty workspace waiting for user action, or a session auto-starts?
...
See [Technical Reference: TypeScript Interfaces](./canon-ide-technical-reference.md#typescript-interfaces) for the interface definitions and XML output format.
...
**Session Lifecycle**
Start a new Claude Code session from Canon. Send prompts (composed from annotations or typed directly). See streaming output. Session persists until explicitly ended.

**Live File Change Detection**
When Claude Code edits a file, Canon surfaces the diff in the code panel immediately — no manual refresh needed. The stream-json output includes structured tool results with file paths and diffs, which is a better data source than filesystem watching.
...
When multiple sessions touch the same file, Canon surfaces the conflict. Annotations can reference what another session did: "Session 1 already refactored this — coordinate."

### Tier 3: Advanced Interaction

**Annotation-Driven Prompting**
Skip the text input entirely. Browse code → annotate → submit. Canon composes the prompt. This is already what `/canon:new` does, elevated to the primary interaction model.

**Tool Call Review**
Tool calls are structurally different from text messages — they have a clear action, a result, and reversibility context. Canon renders each tool call as a rich, annotatable block rather than opaque text: file edits show inline diffs, Bash commands show exit codes, Write calls show the created content.

Annotating a tool call anchors to the specific `toolUseId`, not a text selection. This enables precise correction workflows:
- **Post-hoc correction**: "This edit was wrong — the import should be from `./utils` not `../utils`." Canon composes the annotation with the tool call context so Claude knows exactly which edit to revisit.
- **Undo request**: Annotate a destructive action with "Revert this." The stream-json output includes `tool_use_result.originalFile` and `structuredPatch`, so Canon can show what was lost and compose a revert prompt.
- **Cross-panel linking**: Click a tool call that edited `auth.ts` and the code panel jumps to the file, highlighting the changed lines. Annotate either surface — the tool call in the conversation or the lines in the code panel — and Canon understands they're related.

Future possibility: if Canon ever moves to the Agent SDK path, tool calls could be intercepted *before* execution for approval/modification. Under the current CLI subprocess model, review is post-hoc only.
...
**Session Forking**
See a conversation going well until turn 5? Fork it. Claude Code supports session resumption and forking; Canon could create branching conversation paths. See [Technical Reference: Discovered Flags](./canon-ide-technical-reference.md#discovered-flags-useful-for-canon) for the relevant CLI flags.
...
The transition to Canon-as-IDE replaces the current one-shot model entirely. The following will be removed:

- **`/canon:new` and `/canon:setup` slash commands** — Canon is no longer invoked from within Claude Code
...
- Canon becomes long-running (persistent mode is the default, no flag needed)
- Integrate conversation rendering into the existing Canon UI alongside code/diff panels
- New `ConversationPanel` component renders structured messages with Canon's design language
...
| Tool | Primary Model | Canon-as-IDE |
|------|--------------|--------------|
...
| **Canon** | **Annotation-first**: review + directed feedback | **This** |
...
See [Technical Reference](./canon-ide-technical-reference.md) for implementation-level mitigations (WebSocket reconnection, stream-json error detection).
